...
#SBATCH --job-name=gen_matrix_train_s10_19
#SBATCH --array=0-149

# TRAINING PHASE - SEEDS 10-19 (Additional Run)
# 150 tasks: 15 conditions × 10 seeds
# Each task trains one model and saves checkpoint
# Uses seeds 10-19 (different from initial run which used seeds 0-9)
...
# Nested array calculation with seed offset
NUM_SEEDS=10
SEED_BASE=6431
SEED_GAP=10
SEED_OFFSET=10  # Offset to get seeds 10-19 instead of 0-9

CONDITION_ID=$((SLURM_ARRAY_TASK_ID / NUM_SEEDS))
SEED_ID_RAW=$((SLURM_ARRAY_TASK_ID % NUM_SEEDS))
SEED_ID=$((SEED_ID_RAW + SEED_OFFSET))  # Shift to 10-19
...
========================================
TRAINING PHASE - SEED MANIFEST (SEEDS 10-19)
========================================
...
Random Seed:         ${SEED}
Phase:               TRAINING
Seed Range:          10-19 (Additional Run)
----------------------------------------
Node:                ${SLURM_NODELIST}
Start Time:          $(date)
...
echo "=========================================="
echo "TRAINING PHASE (SEEDS 10-19) - Task ${SLURM_ARRAY_TASK_ID}"
echo "Condition: ${CONDITION_ID}, Seed ID: ${SEED_ID}, Seed: ${SEED}"
echo "=========================================="
...
echo "Training task ${SLURM_ARRAY_TASK_ID} (Seed ${SEED_ID}) completed (exit: ${EXIT_STATUS})"
