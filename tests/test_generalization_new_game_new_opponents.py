#!/usr/bin/env python3
"""
Train on PD with opponents [0.1, 0.3], test on HD with opponents [0.7, 0.9].
"""
...
def test_pd_train_hd_test_new_opponents():
    setup_logging("INFO")
    print("=== TEST: PD train -> HD test (new opponents) ===")
    ...
    test_opponents = OpponentFactory.create_opponent_set([0.7, 0.9])
    ...
    report_path = os.path.join(output_dir, 'pd_train_hd_test_new_opponents.json')
    with open(report_path, 'w') as f:
        json.dump({
            'train_game': 'prisoners-dilemma',
            'test_game': 'hawk-dove',
            'train_opponents': [0.1, 0.3],
            'test_opponents': [0.7, 0.9],
...
if __name__ == "__main__":
    test_pd_train_hd_test_new_opponents()
    print("✅ PD train -> HD test (new opponents) completed")
